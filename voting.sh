...
docker run --restart=unless-stopped --name frontend01 -e affinity:container==redis01 -d -e WEB_VOTE_NUMBER="01" --hostname votingapp.local -p 5000:80 containerx/web-vote-app
docker run --restart=unless-stopped --name frontend02 -e affinity:container==redis02 -d -e WEB_VOTE_NUMBER="02" --hostname votingapp.local -p 5000:80 containerx/web-vote-app
...
docker run --restart=unless-stopped -e "affinity:container==store" -p 3406:80 --name results-app --hostname votingresult.local -d containerx/results-app
